...
- The tunnel binding owns ordinary tunnel CRUD. A second account token with the same Cloudflare
  Tunnel Write permission exists only because the official forced-connection cleanup endpoint is
  not exposed by that binding. Its bearer value is confined to the private control Worker, is used
  only for `DELETE /connections`, and deliberately duplicates no DNS or application authority.

...
This repository is a clean-slate implementation. Do not inspect, cite, copy, or derive decisions from any previous Glass codebase.

## Product non-negotiables

1. **The product is always available.** Identity, projects, artifacts, conversations, shared documents, and collaboration belong to Glass Cloud. A user can open Glass and work with cloud-owned state while every execution environment is offline.
...
- **project**: a cloud-owned container for work, artifacts, threads, documents, and execution associations. A project is not a filesystem directory.
...
- **document**: a shared, cloud-owned collaborative document.
...
- **Glass Connect**: the future managed, authenticated path between Glass Cloud and an execution node. It is not implemented by the foundation.
...
1. **Making execution the product backend.** The API must not proxy ordinary product state through an execution node. An offline node cannot make projects, conversations, documents, or identity unavailable.
...
The foundation intentionally exposes no fake login or protected product endpoint. See `docs/internals/environment-auth.md` for the future environment trust model.
...
- Do not send full documents, transcripts, directory trees, or repository state when a delta or page is sufficient.
...
A required Milestone 0 or 1 path is complete only when it is implemented, wired into the workspace, and verified. Completion never includes:
